...
export const addCheckOut = async (req, res, next) => {
  try {
    ...
    const checkOutData = {
      cart: {
        items: cart.items.map((item) => ({
          menu: item.menuItemId, // Expecting menuItemId to be sent in the body
          quantity: item.quantity,
        })),
    ...
    if (paymentMethod === "creditCard") {
      ...
      const orderMenuItems = cart.items.map((item) => ({
        name: item.name, // You should ensure 'name' is passed in the request or populated
        price: item.price,
        quantity: item.quantity,
      }));
...
//     //   "cart.items.menu"
//     // );
//     const checkOuts = await CheckOutModel.find({ user: userId })
